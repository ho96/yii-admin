...
class Message
{
	...
	public static function findAll($language, $file)
	{
		...
		if($language != '' && $file != '')
		{
			$handle = fopen(Yii::app()->basePath . '/messages/' . $language . '/' . $file, "r");
			if ($handle) {
				$i = 0;
	...
	{
		$fileContents = '';
		$handle = fopen(Yii::app()->basePath . '/messages/' . $this->language . '/' . $this->file, "r");
		if ($handle) {
			$i = 0;
		    while (($line = fgets($handle)) !== false) {
		        if(strpos($line,'\'') > -1 && strpos($line,'=>') > -1)
		...
		file_put_contents(Yii::app()->basePath . '/messages/' . $this->language . '/' . $this->file, $fileContents);		
	}
